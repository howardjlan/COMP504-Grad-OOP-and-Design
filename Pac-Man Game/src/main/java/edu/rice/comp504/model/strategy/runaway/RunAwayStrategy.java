package edu.rice.comp504.model.strategy.runaway;
...
public class RunAwayStrategy implements IRunAwayStrategy {
    private static IUpdateStrategy singleton;

    /**
     * Make a run away strategy.  There is only one (singleton).
     * @return A run away strategy
     */
    public static IUpdateStrategy makeStrategy() {
        if (singleton == null) {
            singleton = new RunAwayStrategy();
        }
    ...
    public void updateState(ACharacter pacman, ACharacter context) {
        ...
        if (pacman.getName().equals("pacman")) {
            ...
            if (ghost.isFlashing()) {
                ghost.setFlashingTimer(ghost.getFlashingTimer() - DispatcherAdapter.updatePeriod);
            }
