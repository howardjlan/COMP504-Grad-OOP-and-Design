...
/**
 * The dumb chase strategy causes the ghost to wander around an area.
 */
public class PatrolChaseStrategy implements IChaseStrategy {
    ...
    public static IUpdateStrategy makeStrategy() {
        if (singleton == null) {
            singleton = new PatrolChaseStrategy();
        }
        ...
        if (context != null) {
            if (pacman.getName().equals("pacman")) {
                ...
                if (context.getName().equals("ghost")) {
                    ...
                    // If the destination needs to be updated, every 60 steps or 10 cells.
                    if (counter > 59) {
                        currentDestination = pacman.getLoc();
                        int distPerStep = (int) Math.floor(2 * pacman.getSize() / pacman.getVel().x);
                        int areaRadius = 5;

                        int randRadius = (int) (Math.random() * areaRadius);
                        int randDir = (int) (Math.random() * 4) + 1;
                        switch (randDir) {
                            case 1:
                                currentDestination = new Point(pacman.getLoc().x - randRadius * distPerStep * pacman.getVel().x, pacman.getLoc().y);
                                break;
                            case 2:
                                currentDestination = new Point(pacman.getLoc().x, pacman.getLoc().y - randRadius * distPerStep * pacman.getVel().y);
                                break;
                            case 3:
                                currentDestination = new Point(pacman.getLoc().x + randRadius * distPerStep * pacman.getVel().x, pacman.getLoc().y);
                                break;
                    ...
                    Point distance = new Point(currentDestination.x - currentLoc.x, currentDestination.y - currentLoc.y);
